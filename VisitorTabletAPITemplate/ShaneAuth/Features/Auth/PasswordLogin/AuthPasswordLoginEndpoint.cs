...
{
    public sealed class AuthPasswordLoginEndpoint : Endpoint<AuthPasswordLoginRequest>
    {
        private readonly UsersRepository _usersRepository;

        public AuthPasswordLoginEndpoint(UsersRepository usersRepository)
        {
        ...
        public override void Configure()
        {
            ...
            SerializerContext(AuthPasswordLoginContext.Default);
            AllowAnonymous();
            Tags("IgnoreAntiforgeryToken");
        }

        public override async Task HandleAsync(AuthPasswordLoginRequest req, CancellationToken ct)
        {
            // Validate request
            ValidateInput(req);
            ...
            (VerifyCredentialsResult result, UserData? userData) = await _usersRepository.VerifyCredentialsAndGetUserAsync(req.Email!, req.Password!, req.TotpCode, "Web", ct);
            ...
            // Create cookie to sign in the user
            await CookieAuth.SignInAsync(userPrivileges =>
            {
        ...
        public void ValidateInput(AuthPasswordLoginRequest req)
        {
            // Trim strings
        ...
        public void ValidateOutput(VerifyCredentialsResult result, UserData? userData)
        {
            switch (result)
            {
                ...
                case VerifyCredentialsResult.TotpCodeRequired:
                    AddError(m => m.TotpCode!, "Two-factor authentication code is required.", "error.login.totpCodeIsRequired");
                    break;
                ...
                case VerifyCredentialsResult.TotpCodeInvalid:
                    AddError(m => m.TotpCode!, "The two-factor authentication code was invalid.", "error.login.totpCodeIsInvalid");
                    break;
