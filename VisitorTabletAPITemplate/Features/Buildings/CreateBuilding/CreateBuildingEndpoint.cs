...
{
    public sealed class CreateBuildingEndpoint : Endpoint<CreateBuildingRequest>
    {
        ...
        public CreateBuildingEndpoint(AppSettings appSettings,
        ...
        {
            Post("/buildings/{organizationId}/create");
            SerializerContext(CreateBuildingContext.Default);
            Policies("User");
            AllowFileUploads();
        }
        ...
        public override async Task HandleAsync(CreateBuildingRequest req, CancellationToken ct)
        {
            ...
            // Validate request
            ContentInspectorResultWithMemoryStream? featureImageContentInspectorResult = await ValidateInputAsync(req, userId.Value);
            ...
            // Query data
            (SqlQueryResult queryResult, Building? building) = await _buildingsRepository.CreateBuildingAsync(req, featureImageContentInspectorResult, userId, adminUserDisplayName, remoteIpAddress);
        ...
        private async Task<ContentInspectorResultWithMemoryStream?> ValidateInputAsync(CreateBuildingRequest req, Guid userId)
        {
            ...
            req.CheckInQRCode = req.CheckInQRCode?.Trim();
            req.AccessCardCheckInWithBookingMessage = req.AccessCardCheckInWithBookingMessage?.Trim();
            req.AccessCardCheckInWithoutBookingMessage = req.AccessCardCheckInWithoutBookingMessage?.Trim();
            req.QRCodeCheckInWithBookingMessage = req.QRCodeCheckInWithBookingMessage?.Trim();
            req.QRCodeCheckInWithoutBookingMessage = req.QRCodeCheckInWithoutBookingMessage?.Trim();
            req.CheckInReminderMessage = req.CheckInReminderMessage?.Trim();
            req.DeskBookingReminderMessage = req.DeskBookingReminderMessage?.Trim();
            ...
            if (req.FeatureImage is not null)
            {
