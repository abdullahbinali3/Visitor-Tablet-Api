...
{
    public sealed class ListImageStorageLogsByCascadeLogIdForDataTableEndpoint : Endpoint<ListImageStorageLogsByCascadeLogIdForDataTableRequest>
    {
        private readonly ImageStorageRepository _imageStorageRepository;
        private readonly UsersRepository _usersRepository;
        private readonly AuthCacheService _authCacheService;

        public ListImageStorageLogsByCascadeLogIdForDataTableEndpoint(ImageStorageRepository imageStorageRepository,
        ...
        {
            Get("/logs/imageStorage/{organizationId}/listLogsByCascadeLogId/{cascadeLogId}");
            SerializerContext(ListImageStorageLogsByCascadeLogIdForDataTableContext.Default);
            Policies("User");
        }

        public override async Task HandleAsync(ListImageStorageLogsByCascadeLogIdForDataTableRequest req, CancellationToken ct)
        {
            ...
            // Query data
            DataTableResponse<ImageStorageLog> imageStorageLogDataTable = await _imageStorageRepository.ListImageStorageLogsByCascadeLogIdForDataTableAsync(req.CascadeLogId!.Value, req.OrganizationId!.Value, req.PageNumber!.Value, req.PageSize!.Value, req.Sort!.Value, req.RequestCounter, SearchQueryParser.SplitTerms(req.Search), ct);

            await SendAsync(imageStorageLogDataTable);
        }

        private async Task ValidateInputAsync(ListImageStorageLogsByCascadeLogIdForDataTableRequest req, Guid userId, CancellationToken cancellationToken)
        {
