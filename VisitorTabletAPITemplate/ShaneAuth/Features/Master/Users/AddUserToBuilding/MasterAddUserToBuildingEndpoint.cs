...
{
    public sealed class MasterAddUserToBuildingEndpoint : Endpoint<MasterAddUserToBuildingRequest>
    {
        ...
        public MasterAddUserToBuildingEndpoint(UserBuildingsRepository userBuildingsRepository,
        ...
        {
            Post("/master/users/addUserToBuilding");
            SerializerContext(MasterAddUserToBuildingContext.Default);
            Policies("Master");
        }

        public override async Task HandleAsync(MasterAddUserToBuildingRequest req, CancellationToken ct)
        {
            ...
            // Query data
            (UserManagementResult queryResult, UserData? userData) = await _userBuildingsRepository.MasterAddUserToBuildingAsync(req, userOrganizationRole!.Value, userId, adminUserDisplayName, remoteIpAddress);
        ...
        private async Task<UserOrganizationRole?> ValidateInputAsync(MasterAddUserToBuildingRequest req)
        {
                ...
                else
                {
                    ...
                    foreach (UserData_UserOrganizations userOrganization in userData!.ExtendedData.Organizations!)
                    {
                        if (userOrganization.Id == req.OrganizationId.Value)
                        {
                            ...
                            // Check if building exists while we are here
                            if (req.BuildingId.HasValue)
                            {
                                foreach (UserData_Building building in userOrganization.Buildings)
                                {
                                    ...
                                    {
                                        HttpContext.Items.TryAdd("FatalError", true);
                                        AddError(m => m.BuildingId!, "The user already belongs to the specified building.", "error.masterSettings.users.buildingExists");
                                        return null;
                                    }
                                }
                            }
            ...
            else if (req.OrganizationId.HasValue)
            {
                if (await _buildingsRepository.IsBuildingExistsAsync(req.BuildingId.Value, req.OrganizationId!.Value))
                {
                    validBuilding = true;
                }
                else
        ...
        private void ValidateOutput(UserManagementResult queryResult, UserData? userData)
        {
            ...
            switch (queryResult)
            {
                case UserManagementResult.Ok:
                    ...
                    return;
                case UserManagementResult.UserAlreadyExistsInBuilding:
                    AddError(m => m.OrganizationId!, "The user already belongs to the specified building.", "error.masterSettings.users.buildingExists");
                    break;
