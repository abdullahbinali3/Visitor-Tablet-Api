...
{
    public sealed class AuthPasswordLoginJwtEndpoint : Endpoint<AuthPasswordLoginJwtRequest, TokenResponse>
    {
        private readonly UsersRepository _usersRepository;

        public AuthPasswordLoginJwtEndpoint(UsersRepository usersRepository)
        {
        ...
        {
            Post("/auth/passwordLoginJwt");
            SerializerContext(AuthPasswordLoginJwtContext.Default);
            AllowAnonymous();
            Tags("IgnoreAntiforgeryToken");
        }

        public override async Task HandleAsync(AuthPasswordLoginJwtRequest req, CancellationToken ct)
        {
            // Validate request
            ValidateInput(req);
            ...
            (VerifyCredentialsResult result, UserData? userData) = await _usersRepository.VerifyCredentialsAndGetUserAsync(req.Email!, req.Password!, req.TotpCode, "Mobile", ct);
        ...
        public void ValidateInput(AuthPasswordLoginJwtRequest req)
        {
            // Trim strings
