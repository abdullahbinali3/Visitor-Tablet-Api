...
{
    public sealed class ListBuildingsAndFunctionsForDropdownsEndpoint: Endpoint<ListBuildingsAndFunctionsForDropdownsRequest>
    {
        private readonly OrganizationsRepository _organizationsRepository;
        private readonly AuthCacheService _authCacheService;

        public ListBuildingsAndFunctionsForDropdownsEndpoint(OrganizationsRepository organizationsRepository,
        ...
        {
            Get("/organizations/{id}/listBuildingsAndFunctionsForDropdowns");
            SerializerContext(ListBuildingsAndFunctionsForDropdownsContext.Default);
            Policies("User");
        }

        public override async Task HandleAsync(ListBuildingsAndFunctionsForDropdownsRequest req, CancellationToken ct)
        {
            ...
            // Validate request
            await ValidateInputAsync(req.id, userId.Value);
            ...
            // Query data
            (SqlQueryResult queryResult, ListBuildingsAndFunctionsForDropdownsResponse? response)
                = await _organizationsRepository.ListBuildingsAndFunctionsForDropdownsAsync(req, ct);
