...
{
    public sealed class ListImageStorageLogsForDataTableEndpoint : Endpoint<ListImageStorageLogsForDataTableRequest>
    {
        private readonly ImageStorageRepository _imageStorageRepository;
        private readonly UsersRepository _usersRepository;
        private readonly AuthCacheService _authCacheService;

        public ListImageStorageLogsForDataTableEndpoint(ImageStorageRepository imageStorageRepository,
        ...
        {
            Get("/logs/imageStorage/{organizationId}/listLogs");
            SerializerContext(ListImageStorageLogsForDataTableContext.Default);
            Policies("User");
        }

        public override async Task HandleAsync(ListImageStorageLogsForDataTableRequest req, CancellationToken ct)
        {
            ...
            // Query data
            DataTableResponse<ImageStorageLog> imageStorageLogDataTable = await _imageStorageRepository.ListImageStorageLogsForDataTableAsync(req.OrganizationId!.Value, req.PageNumber!.Value, req.PageSize!.Value, req.Sort!.Value, req.RequestCounter, SearchQueryParser.SplitTerms(req.Search), ct);

            await SendAsync(imageStorageLogDataTable);
        }

        private async Task ValidateInputAsync(ListImageStorageLogsForDataTableRequest req, Guid userId, CancellationToken cancellationToken)
        {
